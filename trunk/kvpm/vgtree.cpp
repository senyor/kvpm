...
QTreeWidgetItem *VGTree::loadItem(LogVol *lv, QTreeWidgetItem *item)
{
    ...
    */
    ...
    if (is_sc) {
        ...
        if (lv->isActive()) {
            if ((lv->isThinPool() || lv->isThinVolume()) && !lv->isSnapContainer())
                item->setData(7, Qt::DisplayRole, QString("%%1").arg(lv->getDataPercent(), 1, 'f', 2));
            else if ((lv->isPvmove() || lv->isRaid() || lv->isMirror()) && !lv->isSnapContainer())
                item->setData(7, Qt::DisplayRole, QString("%%1").arg(lv->getCopyPercent(), 1, 'f', 2));
            else if (lv->isCowSnap() || lv->isMerging())
        ...
        if (!lv->isSnapContainer() && !lv->isLvmMirrorLog() && !lv->isLvmMirrorLeg() && 
            ...
            !lv->isThinPool()      && !lv->isThinPoolData()) {
